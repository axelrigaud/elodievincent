jQuery(document).ready(function() {
  ...
  var Portfolio = (function () {
    ...
    return {
      ...
      behanceProjectsAPI: function () {
        return 'http://www.behance.net/v2/users/'+ s.userID +'/projects?callback=?&api_key='+ s.apiKey+'&per_page=25';
      },

      behanceProjectAPI: function (id) {
        return 'http://www.behance.net/v2/projects/'+id+'?callback=?&api_key='+ s.apiKey;
      },
      ...
      setModalTemplate: function (id) {
        ...
        var rawProjectData = JSON.parse(sessionStorage.getItem('behanceProject'+id)),
          ...
          projectData = this.prepareProject(rawProjectData),
      ...
      showProject: function (id) {
        if(sessionStorage.getItem('behanceProject'+id)){
          this.setModalTemplate(id);
        }
        else {
          var that = this;
          $.getJSON(this.behanceProjectAPI(id),function(project){
            var data = JSON.stringify(project);
            sessionStorage.setItem('behanceProject'+id,data);
            that.setModalTemplate(id);
          })
        }
      },
      ...
      createLinks: function () {
        ...
        $('.portfolio-item a').on('click', function(e){
          e.preventDefault();
          that.showProject($(this).data("project_id"));
        });
      },
      ...
      init: function () {
        ...
        $(".submit-btn").click(function() {
          debugger;
            ...
            if (that.clientSideValidation()) //everything looks good! proceed...
            {
                console.log('valid');
